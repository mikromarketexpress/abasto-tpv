...
import { TrendingUp, DollarSign, ArrowUpRight, ArrowDownRight, Package, Clock, RefreshCw, AlertTriangle } from 'lucide-react'
...
const getResumenCuentas = (cuentas) => {
    const resultado = { critico: 0, proximo: 0, totalPendiente: 0 }
    const hoy = dayjs()
    cuentas.forEach(c => {
        if (c.saldoPendiente <= 0) return
        const dias = dayjs(c.fechaVencimiento).diff(hoy, 'day')
        if (dias <= 3) resultado.critico++
        else if (dias <= 7) resultado.proximo++
        resultado.totalPendiente += c.saldoPendiente
    })
    return resultado
}
...
const Dashboard = ({ setActivePage }) => {
    ...
    return (
        <div className="s-scroll" style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: '1.25rem', paddingRight: '1rem' }}>
            ...
            {(() => {
                const resumenCobrar = getResumenCuentas(CUENTAS_COBRAR)
                const resumenPagar = getResumenCuentas(CUENTAS_PAGAR)
                ...
                return (
                    ...
                    >
                        <AlertTriangle size={20} style={{ color: '#ff3131', flexShrink: 0 }} />
                        <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap', flex: 1 }}>
                            {resumenCobrar.critico > 0 && (
                                ...
                                >
                                    <span style={{ fontSize: '0.8rem', color: '#ff3131', fontWeight: 700 }}>
                                        {resumenCobrar.critico} Cuentas por Cobrar CRÍTICAS
                                    </span>
                                    <span style={{ fontSize: '0.7rem', color: 'var(--s-text-dim)' }}>
                            ...
                            {resumenCobrar.proximo > 0 && (
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <span style={{ fontSize: '0.8rem', color: '#ffc107', fontWeight: 700 }}>
                                        {resumenCobrar.proximo} Próximas a vencer
                                    </span>
                                </div>
                            )}
                            {resumenPagar.critico > 0 && (
                                <button 
                                    ...
                                    style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem' }}
                                >
                                    <span style={{ fontSize: '0.8rem', color: '#ff6b6b', fontWeight: 700 }}>
                                        {resumenPagar.critico} Cuentas por Pagar CRÍTICAS
                                    </span>
                                    <span style={{ fontSize: '0.7rem', color: 'var(--s-text-dim)' }}>
                        ...
                        <span style={{ fontSize: '0.7rem', color: 'var(--s-text-dim)', fontWeight: 600 }}>
